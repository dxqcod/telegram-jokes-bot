...
При запуске бот показывает приветственное изображение. Оно остаётся в поле чата и не исчезает, даже когда остальные сообщения удаляются. Таким образом, у пользователя всегда есть «якорь» — красивая картинка и кнопка.
